...
app.get('/', function(req, res) {
  res.render('index', {
    locals: {
      scripts: ['front-page.js'],
      games: req.session.games || []
    }
  });
});

app.get(/\/api\/game-words\/([345])/, function(req, res) {
...
app.post('/api/save-game', function(req, res) {
  ...
  game.time = _.reduce(game.times, function(memo, num){ return memo + num; }, 0);
  game.changes = game.words.length+1;
